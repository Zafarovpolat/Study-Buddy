...
class GroupService:
    ...
    async def _count_user_groups(self, user: User) -> int:
        ...
        result = await self.db.execute(
            select(func.count(Folder.id)).where(
                Folder.user_id == user.id, Folder.is_group == True
            )
        )
    ...
    async def create_group(
        self, owner: User, name: str, description: Optional[str] = None
        ...
        if current_groups >= max_groups:
            if owner.effective_tier == SubscriptionTier.SOS:
                ...
                return (
                    False,
                    f"Лимит групп исчерпан ({max_groups}). Оформите Pro для создания до 30 групп.",
                    None,
                )
            else:
        ...
        group = Folder(
            ...
            is_group=True,
            max_members=owner.max_members_per_group
            if owner.max_members_per_group < 999999
            else 9999,
        )
        ...
        membership = GroupMember(
            group_id=group.id, user_id=owner.id, role=GroupRole.OWNER
        )
    ...
    async def join_group(
        ...
        if current_members >= max_members:
            if max_members <= 5:
                return (
                    False,
                    f"Группа заполнена (макс. {max_members} участников в Free)",
                    group,
                )
            return False, "Группа заполнена", group

        membership = GroupMember(
            group_id=group.id, user_id=user.id, role=GroupRole.MEMBER
        )
        self.db.add(membership)
        await self.db.commit()
    ...
    async def can_add_material_to_group(
        ...
        if materials_count >= max_materials:
            if max_materials <= 10:
                return (
                    False,
                    f"Лимит материалов в группе ({max_materials}). Владельцу нужен Pro.",
                )
    ...
    async def get_user_groups(self, user: User) -> List[dict]:
        """Получить все группы пользователя — ОДИН запрос с GROUP BY"""
        # Один запрос: группы + количество участников + количество материалов
        result = await self.db.execute(
            select(
                GroupMember,
                Folder,
                func.count(func.distinct(GroupMember.id))
                .over(partition_by=GroupMember.group_id)
                .label("member_count"),
                func.count(func.distinct(Material.id))
                .over(partition_by=Folder.id)
                .label("material_count"),
            )
            .join(Folder, GroupMember.group_id == Folder.id)
            .outerjoin(Material, Material.folder_id == Folder.id)
        ...
        seen_groups = set()
        groups = []
        for membership, folder, member_count, material_count in result.all():
            if folder.id in seen_groups:
                continue
            seen_groups.add(folder.id)
            ...
            groups.append(
                {
                    ...
                    "materials_count": material_count or 0,
    ...
    async def process_referral(
        self, new_user: User, referral_code: str
        ...
        if (
            referrer.referral_count >= self.REFERRAL_PRO_THRESHOLD
            and not referrer.referral_pro_granted
        ):
    ...
    async def _grant_referral_pro(self, user: User) -> None:
        ...
        user.subscription_expires_at = datetime.now() + timedelta(
            days=self.REFERRAL_PRO_DAYS
        )
    ...
    async def get_referral_stats(self, user: User) -> dict:
        ...
        return {
            ...
            "referrals_needed": max(
                0, self.REFERRAL_PRO_THRESHOLD - (user.referral_count or 0)
            ),
